...
func DeleteBoard(boardRepo *repository.BoardRepository, columnRepo *repository.ColumnRepository, recordRepo *repository.RecordRepository) gin.HandlerFunc {
	return func(c *gin.Context) {
		boardID := c.Param("id")

		board, err := boardRepo.GetBoardByDelID(boardID)
		if err != nil {
			c.JSON(http.StatusNotFound, gin.H{"error": "Board not found"})
			return
		}
